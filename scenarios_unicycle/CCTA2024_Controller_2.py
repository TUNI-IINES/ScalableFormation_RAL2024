...
from control_lib.cbf_single_integrator_2 import cbf_si, np
...
class SceneSetup:
    ...
    form_A: np.array = np.array([[0, 0.5, 0, 0],
                                 ...
                                 [0, 0, 0, 0.5],
                                 [0, 0, 0.5, 0]])
    form_A_scale: np.array = form_A.copy()  # adjacent matrix for epsilon
    ...
    scale_gain: float = 1.  # for achieving eps → 0
    scale_weight: float = 1.  # priority in optimizing function
    initial_scale: float = 1.0  # ratio
    epsilon = 0.1
...
class Controller:
    ...
    def __init__(self):
        ...
        # Setup cbf
        self.cbf = [cbf_si(scale=SceneSetup.form_A_scale[i]) for i in range(SceneSetup.robot_num)]
    ...
    def compute_control(self, feedback, computed_control):
        ...
        for i in range(SceneSetup.robot_num):
            ...
            i_scale = feedback.get_robot_i_scale(i)
            ...
            Ni = len(self.__inNeigh[i]) + 1
            s_nom = 0.01 * (1 - i_scale) + i_scale / Ni - i_scale

            # Formation Maintenance
            if SceneSetup.USECBF_FORMATION:
                for j in self.__inNeigh[i]:
                    j_q = feedback.get_robot_i_pos(j)
                    j_scale = feedback.get_robot_i_scale(j)
                    if SceneSetup.use_unicycle:
                        j_q = feedback.get_lahead_i_pos(j)

                    h_fml, h_fmu, h_scale_floor, h_scale_ceil = self.cbf[i].add_maintain_distance_with_scale(
                        current_q, j_q, SceneSetup.form_A[i, j], SceneSetup.max_form_dist[i][j], 2 * SceneSetup.d_obs,
                        i_scale, j_scale, SceneSetup.epsilon, gamma=500, power=3)

                    # store h value
                    # TODO: also change in pickleplot
                    computed_control.save_monitored_info(f"h_eps_ceil_{i}_{j}", h_scale_ceil)
                    computed_control.save_monitored_info(f"h_eps_floor_{i}_{j}", h_scale_floor)
                    computed_control.save_monitored_info(f"h_fml_{i}_{j}", h_fml)
                    computed_control.save_monitored_info(f"h_fmu_{i}_{j}", h_fmu)

                    s_nom += j_scale / Ni
                    # print(i, j, h_fml, h_fmu, h_scale_floor, h_scale_ceil)
            ...
            # Ensure safety
            # print(f'Robot {i}')
            s_nom = SceneSetup.scale_gain * s_nom
            u, s = self.cbf[i].compute_safe_controller(u_nom, s_nom,
                                                       weight=SceneSetup.scale_weight)
            ...
            i_scale = self.cbf[i].get_additional_state()
            for idx, k in enumerate(i_scale.tolist()):
                computed_control.save_monitored_info(f"eps_{i}_{idx}", k)
            computed_control.set_i_scale(i, i_scale)
            print(i, i_scale, s_nom, s)
...
class ControlOutput:
    ...
    def __init__(self):
        ...
        self.__all_scale = SceneSetup.form_A_scale.copy()
        self.reset_monitor()
    ...
    def get_all_scale(self):
        ...
        return self.__all_scale.copy()

    def get_i_scale(self, ID):
        """
        __all_eps getter
        :return: Nx1 vector, epsilon of each robot
        """
        return self.__all_scale[ID]
    ...
    def set_i_scale(self, ID, input_scale):
        """
        __all_eps @ row ID-th setter

        :param ID: integer, ID-th robot in the simulation
        :param input_scale: scalar, new epsilon
        """
        self.__all_scale[ID] = input_scale
...
class FeedbackInformation:
    ...
    def __init__(self):
        ...
        self.__all_robot_scale = SceneSetup.form_A_scale.copy()
        ...
        self.__all_robot_scale_hist = self.__all_robot_scale.copy().reshape(SceneSetup.form_A_scale.shape[0], 1)
    ...
    def set_feedback(self, all_robots_pos, all_robots_theta, all_robots_scale=None):
        """
        ...
        if all_robots_scale is not None:
            self.__all_robot_scale = all_robots_scale.copy()
    ...
    def get_robot_i_scale(self, i):
        ...
        """
        return self.__all_robot_scale.copy()[i]

    def get_robot_i_scale_hist(self, i):
        """
        ...
        """
        return self.__all_robot_scale_hist.copy()[i, :]
    ...
    def set_all_scale(self, scales):
        """
        __all_robot_epsilon setter
        :param epsilons: Nx1 vector, new epsilons
        """
        # print("v:\n", self.__all_robot_epsilon - epsilons)
        self.__all_robot_scale = scales
        self.__all_robot_scale_hist = np.hstack((self.__all_robot_scale_hist, scales.reshape(4, 1)))
