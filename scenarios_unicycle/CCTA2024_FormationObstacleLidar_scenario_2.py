...
from scenarios_unicycle.CCTA2024_Controller_2 import SceneSetup, np
...
def import_scenario(filename="formation4_exp_2", directory="scenarios_unicycle/scenarios/"):
    ...
    SceneSetup.max_form_dist = block_diag(*[np.array(mfe) for mfe in scenario['formations']['max_form_distance']])
    ...
    SceneSetup.form_A_scale = SceneSetup.initial_scale * np.array([1 for _ in range(SceneSetup.robot_num)])
    SceneSetup.form_A_edges = np.count_nonzero(SceneSetup.form_A, axis=1)
...
class SimulationCanvas:
    ...
    def update_simulation(self, control_input, feedback):
        if self.__cur_time < SimSetup.tmax:
            ...
            if SimSetup.scale_visualization:  # TODO WIDHI: check notes in SimSetup
                scale_array = control_input.get_all_scale()
                feedback.set_all_scale(scale_array)
    ...
    def __initiate_plot(self):
            ...
            for j in range(SceneSetup.robot_num):
                if (i < j) and (SceneSetup.form_A[i, j] > 0):
                    self.__drawn_comm_lines[str(i) + '_' + str(j)], = ax_2D.plot([-i, -i], [j, j],
                                                                                 color='k', linewidth=0.15 / SceneSetup.max_form_dist[i][j])
                ...
                if (i < j) and (SceneSetup.form_A[i, j] > 0):
                    self.__drawn_distance_lines[str(i) + '_' + str(j)], = self.__ax_dist.plot(0, 0,
                                                                                              ...
                                                                                              label='$i={},\ j={}$'.format(
                                                                                                  i + 1, j + 1))
                    cnt += 1
        self.__drawn_scale_lines = {}
        cnt = 0
        for i in range(SceneSetup.robot_num):
            for j in range(SceneSetup.robot_num):
                if (i < j) and (SceneSetup.form_A[i, j] > 0):
                    self.__drawn_scale_lines[str(i) + '_' + str(j)], = self.__ax_dist.plot(0, 0,
                                                                                           linestyle='dashed',
                                                                                              color=colorList[cnt],
                                                                                              label='$ki={},\ kj={}$'.format(
                                                                                                  i + 1, j + 1)),
        ...
        array_max_scale = np.unique(SceneSetup.max_form_dist)
        array_max_scale = np.delete(array_max_scale, 0)
        ...
        # set y-axis
        self.__ax_dist.set(ylim=(max(min(array_req_dist) - max(array_max_scale) - 0.1, 0.0),
                                 max(array_req_dist) + max(array_max_scale) + 0.1))
            ...
            for j in range(SceneSetup.robot_num):
                if (i < j) and (SceneSetup.form_A[i, j] > 0):
                    ...
                    if SimSetup.scale_visualization:  # TODO WIDHI: check notes in SimSetup
                        ...
                        scale_hist = (2 - feedback.get_robot_i_scale_hist(i)[-visible_length:] + \
                                   feedback.get_robot_i_scale_hist(j)[-visible_length:]) / 2 * SceneSetup.form_A[i, j]

                        self.__drawn_scale_lines[str(i) + '_' + str(j)].set_data(
                            log_data['time'][min_idx:max_idx], scale_hist)
                        # fill_instance = self.__ax_dist.plot(
                        #     np.linspace(max(0, self.__cur_time - SimSetup.timeseries_window), self.__cur_time,
                        #                 num=scale_hist.size),
                        #     scale_hist, alpha=0.1, linewidth=1, label=f'Scale{i}{j}')
