...
class NebolabSetup:
    ...
    # -----------------------------------------------------------------
    SCALE_M2PXL = 363.33
    # PXL_OFFSET = [0 ,0] # for Left camera in pixel
    ...
    def pos_pxl2m(px, py):
        return float((px - NebolabSetup.PXL_ORIGIN[0] ) /NebolabSetup.SCALE_M2PXL), float \
            (-(py - NebolabSetup.PXL_ORIGIN[1] ) /NebolabSetup.SCALE_M2PXL)
    
    @staticmethod
    def skewed_pos_m2pxl(x, y):
        return NebolabSetup.PXL_ORIGIN[0] + int( x * NebolabSetup.SCALE_M2PXL * 1.05), NebolabSetup.PXL_ORIGIN[1] - int( y * NebolabSetup.SCALE_M2PXL * 1.05)
