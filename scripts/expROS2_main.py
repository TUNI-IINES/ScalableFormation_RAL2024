...
class Computation(Node):
    ...
    def control_loop(self):
        ...
        if diff > (1.1*self.Ts): # Add 10% extra margin
            self.get_logger().info('WARNING loop rate is slower than expected. Period (ms): {:0.2f}'.format(diff*1000 ))     
        self.check_t = now
        ...
        if all (v is not None for v in self.environment.global_poses[ :SceneSetup.robot_num ]):
            ...
            self.environment.update_feedback( self.feedback_information, self.control_input)
